export const authMessages = {
	...
	error: {
		...
		emailVerification: {
			invalidToken:
				"Email verification failed. The verification token is invalid or expired.",
...
export const crudMessages = {
	...
	error: {
		create: {
			validationError: (item: string) =>
				`Creation of ${item} failed. Validation error occurred.`,
			...
			duplicateEntry: (item: string) =>
				`Creation of ${item} failed. Duplicate entry detected.`,
		...
		update: {
			...
			validationError: (item: string) =>
				`Update of ${item} failed. Validation error occurred.`,
		...
		delete: {
			...
			dependencyError: (item: string) =>
				`Deletion of ${item} failed. Dependency error occurred.`,
...
export const zodMessages = {
	error: {
		...
		limit: {
			stringMin: (field: string, limit: number) =>
				`${field} must be at least ${limit} characters.`,
			stringMax: (field: string, limit: number) =>
				`${field} must not exceed ${limit} characters.`,
			arrayMin: (field: string, limit: number) =>
				`${field} must have at least ${limit} items.`,
		...
		invalid: {
			...
			invalidFileSize: (field: string, limit: number) =>
				`${field} must not exceed ${limit} bytes.`,
